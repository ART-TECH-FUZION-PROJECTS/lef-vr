...
function lef_submit_reservation() {
	...
	// ── Send Centralized Emails via LEF_Email_Controller ──
	$email_controller = new LEF_Email_Controller();
	$res_id           = $wpdb->insert_id;

	// Send to Admin
	$admin_email = get_option( 'admin_email' );
	$email_controller->send_email( 'reservation_request', $admin_email, array(
		'reservation_id' => $res_id,
		'recipient_type' => 'admin',
	) );

	// Send to User
	$email_controller->send_email( 'reservation_request', $user_email, array(
		'reservation_id' => $res_id,
		'recipient_type' => 'user',
	) );
...
function lef_reserv_update_status() {
	...
	if ( $updated !== false ) {
		// If status is updated to completed, send thank you and review email to traveller
		if ( $status === 'completed' ) {
			...
			if ( $reservation ) {
				...
				if ( $user_info ) {
					$traveller_email = $user_info->user_email;
					$traveller_name  = $user_info->display_name;

					if ( function_exists( 'lef_get_secure_detail_url' ) ) {
						$property_url = lef_get_secure_detail_url( $reservation->property_id );
					} else {
						$property_url = home_url( '/property/' . $reservation->property_id );
					}

					// Append #reviews to redirect directly to the review section
					$review_url = $property_url . '#reviews';

					$email_controller = new LEF_Email_Controller();
					$email_controller->send_email( 'reservation_completed', $traveller_email, array(
						'user_name'     => $traveller_name,
						'property_name' => $reservation->property_title,
						'property_url'  => $review_url,
...
function lef_edit_prof_send_otp() {
	...
	// Determine specific OTP trigger
	$current_phone       = get_user_meta( $user_id, 'mobile_number', true );
	$current_phone_clean = str_replace( ' ', '', $current_phone );
	$new_phone_clean     = str_replace( ' ', '', $new_phone );

	$is_phone_changed = ( ! empty( $new_phone ) && $new_phone_clean !== $current_phone_clean );
	$is_email_changed = ( ! empty( $new_email ) && $new_email !== $user->user_email );
	$is_pass_changed  = ( ! empty( $new_password ) );

	$trigger = 'otp_general_update';
	if ( $is_phone_changed && ! $is_email_changed && ! $is_pass_changed ) {
		$trigger = 'otp_mobile_update';
	} elseif ( $is_email_changed && ! $is_phone_changed && ! $is_pass_changed ) {
		$trigger = 'otp_email_update';
	} elseif ( $is_pass_changed && ! $is_email_changed && ! $is_phone_changed ) {
		$trigger = 'otp_password_update';
	}
	...
	// ── Send Centralized Emails via LEF_Email_Controller ──
	$email_controller = new LEF_Email_Controller();
	$sent             = $email_controller->send_email( $trigger, $email, array(
		'otp_code'  => $otp,
...
function lef_edit_prof_save_changes() {
	...
	// Track initial changes before committing
	$current_phone       = get_user_meta( $user_id, 'mobile_number', true );
	$current_phone_clean = str_replace( ' ', '', $current_phone );
	$new_phone_clean     = str_replace( ' ', '', $phone );

	$is_phone_changed = ( ! empty( $phone ) && $new_phone_clean !== $current_phone_clean );
	$is_email_changed = ( ! empty( $email ) && $email !== $user->user_email );
	$is_pass_changed  = ( ! empty( $password ) );
	...
	// ── Send Centralized Emails via LEF_Email_Controller ──
	$success_trigger = 'success_general_update';
	if ( $is_phone_updated && ! $is_email_updated && ! $is_pass_updated ) {
		$success_trigger = 'success_mobile_update';
	} elseif ( $is_email_updated && ! $is_phone_updated && ! $is_pass_updated ) {
		$success_trigger = 'success_email_update';
	} elseif ( $is_pass_updated && ! $is_email_updated && ! $is_phone_updated ) {
		$success_trigger = 'success_password_update';
	}

	$email_controller = new LEF_Email_Controller();
	$target_email     = ( $is_email_updated && ! empty( $email ) ) ? $email : $user->user_email;
	$email_controller->send_email( $success_trigger, $target_email, array(
		'user_name' => $user->display_name,
...
/**
 * AJAX handler to send a centralized email triggered by frontend JS.
 * Checks if user is logged in, validates inputs, and triggers the LEF_Email_Controller.
 */
function lef_send_centralized_email() {
	check_ajax_referer( 'lef_myprofile_nonce', 'nonce' );

	if ( ! is_user_logged_in() ) {
		wp_send_json_error( array( 'message' => 'Unauthorized access. Please login first.' ) );
	}

	$email   = isset( $_POST['email'] ) ? sanitize_email( $_POST['email'] ) : '';
	$trigger = isset( $_POST['trigger'] ) ? sanitize_key( $_POST['trigger'] ) : '';

	if ( empty( $email ) || ! is_email( $email ) ) {
		wp_send_json_error( array( 'message' => 'Invalid or missing recipient email address.' ) );
	}

	if ( empty( $trigger ) ) {
		wp_send_json_error( array( 'message' => 'Template trigger ID is required.' ) );
	}

	// Verify if the template exists in config
	$config = lef_get_email_config( $trigger );
	if ( ! $config ) {
		wp_send_json_error( array( 'message' => 'Invalid email trigger template.' ) );
	}

	// Collect any additional dynamic data from POST
	$dynamic_data = array();
	if ( isset( $_POST['dynamic_data'] ) && is_array( $_POST['dynamic_data'] ) ) {
		// Basic sanitization of dynamic data
		foreach ( $_POST['dynamic_data'] as $key => $val ) {
			$dynamic_data[ sanitize_key( $key ) ] = sanitize_text_field( $val );
		}
	}

	$email_controller = new LEF_Email_Controller();
	$sent = $email_controller->send_email( $trigger, $email, $dynamic_data );
